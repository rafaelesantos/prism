    ...
    struct PrismSoftDeleteTests {
        ...
        private func makeDB() async throws -> PrismDatabase {
            ...
            try await db.execute(
                "INSERT INTO items (id, name, deleted_at) VALUES (3, 'deleted1', '2024-01-01T00:00:00Z')")
            try await db.execute(
                "INSERT INTO items (id, name, deleted_at) VALUES (4, 'deleted2', '2024-06-15T12:00:00Z')")
            return db
        }
