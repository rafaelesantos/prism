...
struct RyzeNetworkEndpointTests {
    struct DefaultEndpoint: RyzeNetworkEndpoint {
    ...
    func requestBuildsURLRequestWithEncodableBodyAndTimeout() throws {
        let endpoint = NetworkFixtureEndpoint(
            ...
            headers: [
                "Accept": RyzeNetworkHeaderType.json.rawValue
            ],
            requestBody: .payload(NetworkFixturePayload(query: "ryze")),
            timeoutInterval: 12,
            cacheInterval: 60
        )
    ...
    func requestSupportsStringBodyAndGetCachePolicy() throws {
        let endpoint = NetworkFixtureEndpoint(
            path: "/stream",
            headers: [
                "Content-Type": RyzeNetworkHeaderType.plainText.rawValue
            ],
    ...
    func invalidURLThrowsNetworkError() {
        ...
        do {
            ...
            #expect(error as? RyzeNetworkError == .invalidURL)
        }
    }
